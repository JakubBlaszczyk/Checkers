...
public class MainMenuController {
  ...
  @FXML private Label insertUsername;
  @FXML private TextField username;
  @FXML private Button acceptUsername;
  @FXML private Label insertPort;
  @FXML private TextField localPort;
  @FXML private Button waitButton;
  @FXML private Label waiting;
  ...
  public void initializeServer(String address, Integer port, String nickname) throws IOException {
    ...
    wts =
        new WebTcpClient(
            ...
            address,
            port,
            Base64.getEncoder().encodeToString(nickname.getBytes(StandardCharsets.UTF_8)),
            "dGVzdA==");
  ...
  public void joinActiveGame()
      throws IOException, ExecutionException, InterruptedException, InvitationRejected,
          MoveRejected {
    bQI = new LinkedBlockingQueue<>();
    bQS = new LinkedBlockingQueue<>();
    bQM = new LinkedBlockingQueue<>();
    wts =
        new WebTcpClient(
            ...
            localIP.getText(),
            6969,
            Base64.getEncoder().encodeToString(username.getText().getBytes(StandardCharsets.UTF_8)),
            "dDI=");
    ExecutorService executorService = Executors.newFixedThreadPool(2);
    executorService.submit(wts);
