...
public class ImprovedLogicTest {
  ...
  public void checkBoardNormalMove() {
    try {
      ...
      log.debug("\n{}", logic.toString());
      log.debug("MANDATORY_KILL {}", logic.getTurn().toString());
      assertEquals(new MoveResult(MoveType.NONE).getType(), logic.update(1, 3, 0, 2).getType());
      log.debug("\n{}", logic.toString());
      log.debug("KILL {}", logic.getTurn().toString());
      assertEquals(new MoveResult(MoveType.KILL).getType(), logic.update(2, 2, 7, 7).getType());
      log.debug("--------------------------------------------------");
    } catch (Exception e) {
      log.error("Messegu", e);
    }
  ...
  @Order(1)
  @Test
  public void checkBoardSizeThree() {
    try {
      ImprovedLogic logic = new ImprovedLogic(8, 3);
      log.debug("NONE");
      assertEquals(new MoveResult(MoveType.NONE).getType(), logic.update(0, 0, 1, 1).getType());
      log.debug("\n{}", logic.toString());
      log.debug("NONE");
      assertEquals(new MoveResult(MoveType.NONE).getType(), logic.update(2, 2, 1, 1).getType());
      log.debug("NORMAL {}", logic.getTurn().toString());
      assertEquals(new MoveResult(MoveType.NORMAL).getType(), logic.update(1, 3, 2, 2).getType());
      log.debug("\n{}", logic.toString());
      log.debug("NORMAL {}", logic.getTurn().toString());
      assertEquals(new MoveResult(MoveType.NORMAL).getType(), logic.update(2, 4, 3, 5).getType());
      log.debug("\n{}", logic.toString());
      // log.debug("NONE");
      // assertEquals(new MoveResult(MoveType.NONE).getType(), logic.update(2, 2, 1, 1).getType());
      // log.debug("NONE");
